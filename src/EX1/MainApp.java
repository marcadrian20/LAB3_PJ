...
//            parabolas.add(new Parabola(i->Integer.parseInt(parsedString[i])));
        ...
        for (int i = 0; i < parabolas.size(); i++) {
            System.out.println("Parabola " + (i + 1) + ": " + parabolas.get(i).toString()+"\nVarf: " + Arrays.toString(parabolas.get(i).getVarf()));
        }
        System.out.println("Mijloc seg. care uneste vf. parabolelor 1 si 2:\nSTATIC: "+Arrays.toString(Parabola.getMijloc(parabolas.get(0), parabolas.get(1)))+"\nNON_STATIC: "+Arrays.toString(parabolas.get(0).getMijloc(parabolas.get(1))));
